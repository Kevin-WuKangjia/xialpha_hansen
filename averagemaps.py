...
AGE_ALL = "ageall"
OUTPUT_GROUPS = AGE_GROUPS + (AGE_ALL,)
...
def main() -> None:
    ...
    group_subject_ages: dict[str, dict[str, float]] = {g: {} for g in OUTPUT_GROUPS}
    ...
    for idx, subject_dir in enumerate(subject_dirs, start=1):
        ...
        for source_dir in sorted(subject_dir.iterdir(), key=lambda p: p.name):
            ...
            for feature_dir in sorted(source_dir.iterdir(), key=lambda p: p.name):
                ...
                for target_group in (group, AGE_ALL):
                    key = (target_group, source_name, feature_name)
        ...
        if subject_used:
            ...
            group_subject_ages[AGE_ALL][subject] = age

    feature_summaries: list[dict[str, object]] = []
    written_files = 0

    age_group_feature_items: dict[str, list[tuple[str, str, FeatureAccumulator]]] = {g: [] for g in OUTPUT_GROUPS}
    ...
    total_age_groups = len(OUTPUT_GROUPS)
